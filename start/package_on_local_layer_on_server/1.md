...
```bash
sam local invoke ActivityLogDeletionFunction --env-vars env.json
```

👉 The values from `env.json` are loaded into:

```
process.env
```

Exactly like real AWS Lambda.

---

# ✅ How It Works

If your `env.json` looks like this:

```json
{
  "ActivityLogDeletionFunction": {
...
SAM does this internally:

1. Reads env.json
2. Injects variables into Docker container
3. Makes them available as environment variables
4. Node automatically exposes them via `process.env`

Same behavior as real AWS Lambda.

---

# ⚠ Very Important Rule

Your `env.json` structure MUST match the logical name in template.
...
```json
{
  "dev-cma-activityLogDeletion": { ❌ WRONG
```

Because SAM uses logical ID, not FunctionName.
...
You will see all variables loaded.

---

# 🧠 Real AWS vs Local

| Local (SAM)              | AWS Lambda               |
| ------------------------ | ------------------------ |
| env.json                 | Console Environment tab  |
| Loaded via Docker        | Loaded by AWS            |
| Available in process.env | Available in process.env |

Same usage inside code.
...
Yes ✅
`env.json` variables become available in:
